 ...
 * and CheckView): a whole-course progress gauge on its outer edge, a
 * progress ring + step count for this module, the Read/Quiz step nav,
...
export function LessonRail({
  ...
  return (
    <div className="flex gap-5 sticky top-[92px] max-h-[calc(100vh-120px)]">
      {/* Whole-course completion gauge, always visible alongside the rail
          content (not inside the scroll area) so it reads at a glance. */}
      <div
        role="progressbar"
        aria-label={`Course progress: ${Math.round(coursePct)}% complete`}
        aria-valuenow={Math.round(coursePct)}
        aria-valuemin={0}
        aria-valuemax={100}
        title={`${Math.round(coursePct)}% of the course complete`}
      ...
      <div
        ...
        className="flex flex-col gap-6 flex-1 min-w-0 overflow-y-auto pr-1"
      >
        <div>
